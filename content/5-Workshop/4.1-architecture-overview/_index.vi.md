...
EduTrust là hệ thống hỗ trợ học tập và giám sát thi bằng AI Camera, được triển khai trên AWS để đảm bảo khả năng mở rộng, bảo mật và vận hành ổn định. Workshop này tập trung vào việc thiết lập kiến trúc và luồng triển khai chuẩn cho nhóm.

#### Kiến trúc AWS tổng quan

Internet → Amplify → Application Load Balancer → EC2 Auto Scaling → Backend services

![Kiến trúc EduTrust](/images/2-Proposal/edutrust-architect.png)

#### Thành phần chính (theo lớp)
...
+ **Backend services**: API, xử lý AI, camera events, authentication.
...
+ EC2 Auto Scaling chạy trên nhiều Availability Zone (AZ) để đảm bảo HA.
...
+ **Application Load Balancer**: lớp điều phối, định tuyến request tới backend.
+ **EC2 Auto Scaling**: đảm bảo backend tự mở rộng khi tải tăng.
+ **Backend services**: xử lý nghiệp vụ, AI, camera events và authentication.
